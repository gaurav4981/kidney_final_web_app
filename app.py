...
def load_model():
    ...
    model = load('kidney_xgboost.joblib')  # Load the joblib model named 'kidney3.joblib'
    return model
...
def main():
    ...
    specific_gravity = st.sidebar.number_input('Specific Gravity', min_value=1.0, max_value=2.0, value=1.01, step=0.01)
    albumin = st.sidebar.number_input('Albumin', min_value=0, max_value=5, value=0)
    sugar = st.sidebar.number_input('Sugar', min_value=0, max_value=5, value=0)
    red_blood_cell_count = st.sidebar.number_input('Red Blood Cell Count', min_value=1.0, max_value=10.0, value=5.5, step=0.1, key='red_blood_cell_count')
    ...
    blood_glucose_random = st.sidebar.number_input('Blood Glucose Random', min_value=1, max_value=500, value=100)
    blood_urea = st.sidebar.number_input('Blood Urea', min_value=1, max_value=300, value=50)
    serum_creatinine = st.sidebar.number_input('Serum Creatinine', min_value=0.1, max_value=15.0, value=1.0, step=0.1)
    sodium = st.sidebar.number_input('Sodium', min_value=1, max_value=200, value=135)
    potassium = st.sidebar.number_input('Potassium', min_value=1, max_value=200, value=4)
    haemoglobin = st.sidebar.number_input('Haemoglobin', min_value=1, max_value=20, value=15)
    packed_cell_volume = st.sidebar.number_input('Packed Cell Volume', min_value=1, max_value=60, value=40)
    white_blood_cell_count = st.sidebar.number_input('White Blood Cell Count', min_value=1, max_value=20000, value=9000)
